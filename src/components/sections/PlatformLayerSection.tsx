// Manufacturing OS 4-Layer — Refined with monochrome treatment
...
const LAYERS = [
  ...
  },
  {
    ...
    border: 'border-[var(--text-tertiary)]/20',
        ...
        <div className="mx-auto mt-14 max-w-3xl">
          <AnimateIn delay={0.1}>
            <div className="mb-5 flex items-center gap-3">
              ...
              <span className="text-xl font-black tracking-tight text-[var(--foreground)]">AX Flow</span>
            </div>
          </AnimateIn>
          ...
          <StaggerChildren className="space-y-3" stagger={0.1}>
            {LAYERS.map((layer, i) => (
              <div
                key={layer.name}
                className={`rounded-2xl border ${layer.border} ${layer.bg} p-5 transition-all duration-200 hover:shadow-md sm:p-6`}
              >
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:gap-6">
                  <div className="flex items-baseline gap-2 sm:w-32 sm:shrink-0">
                    <span className={`text-lg font-black tracking-tight ${layer.accent}`}>
                      {layer.name}
                    </span>
                ...
                {i < LAYERS.length - 1 && (
                  <div className="mt-2 text-center text-[var(--text-tertiary)]">
                    ↑
                  </div>
                )}
              </div>
          ...
          {/* + 기존 시스템 */}
          <AnimateIn delay={0.5}>
            <div className="mt-4 flex items-center justify-center gap-2 text-lg font-bold text-[var(--text-tertiary)]">
              +
            </div>
            <div className="mt-2 rounded-2xl border border-dashed border-[var(--border)] bg-[var(--background)] p-5 text-center">
